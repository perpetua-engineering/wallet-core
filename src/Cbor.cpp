...
uint32_t Decode::getTotalLen() const {
    ...
    switch (typeDesc.majorType) {
        ...
        case MT_string:
            {
                // (#4707): attacker-controlled length can overflow uint32_t,
                // defeating the subsequent bounds checks. Reject before the cast.
...
Data Decode::getBytes() const {
    ...
    // (#4707): reject lengths that overflow uint32_t before the bounds check.
    if (typeDesc.value > static_cast<uint64_t>(std::numeric_limits<uint32_t>::max()) - typeDesc.byteCount) {
        throw std::invalid_argument("CBOR bytes/string length overflow");
    }
...
bool Decode::isValid() const {
    try {
        ...
        switch (typeDesc.majorType) {
            ...
            case MT_string:
                {
                    // (#4707): reject overflowing lengths instead of wrapping.
                    if (typeDesc.value > static_cast<uint64_t>(std::numeric_limits<uint32_t>::max()) - typeDesc.byteCount) {
                        return false;
                    }
