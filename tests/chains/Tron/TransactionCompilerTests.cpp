...
TEST(TronCompiler, PreImageHashesRejectsInvalidAddress) {
    auto input = Tron::Proto::SigningInput();
    auto& transaction = *input.mutable_transaction();
    ...
    const auto serializedInput = input.SerializeAsString();
    const auto inputData = Data(serializedInput.begin(), serializedInput.end());
    const auto outputData = TransactionCompiler::preImageHashes(TWCoinTypeTron, inputData);
    ...
    EXPECT_TRUE(output.data_hash().empty());
}
