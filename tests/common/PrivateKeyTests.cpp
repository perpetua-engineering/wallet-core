...
TEST(PrivateKey, SignLongDigest) {
    Data privKeyData = parse_hex("afeefca74d9a325cf1d6b6911d61a65c32afa8e02bd5e78e2e4ac2910bab45f5");
    auto privateKey = PrivateKey(privKeyData);
    Data longDigest = TW::data("123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef012");
    {
        Data actual = privateKey.sign(longDigest, TWCurveSECP256k1);
        EXPECT_EQ(actual.size(), 0ul);
    }
    {
        Data actual = privateKey.sign(longDigest, TWCurveNIST256p1);
        EXPECT_EQ(actual.size(), 0ul);
    }
    {
        Data actual = privateKey.sign(longDigest, TWCurveSECP256k1, isCanonical);
        EXPECT_EQ(actual.size(), 0ul);
    }
}
